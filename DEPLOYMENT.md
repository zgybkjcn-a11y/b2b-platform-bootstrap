...
关系链固定为：**本机开发测试 → GitHub 仓库 → tag/GHCR 镜像 → 局域网服务器生产**。生产当前版本以公开 bootstrap `stable.json`、容器 digest/commit 和 `sudo b2b-platform status` 为准。`v0.2.0` 带迁移 `026`，之后 `v0.2.1`–`v0.2.7` 为无新增 schema 的增长分析与 UI 修复；开发分支可以领先，但未经发布、备份、迁移和健康检查验收的版本不得直接进入生产。
